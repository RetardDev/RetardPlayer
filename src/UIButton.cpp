...
UIButton::UIButton(int x, int y, int w, int h, const std::string &text,
                   const std::function<void()> &onClick, SDL_Renderer *renderer,
                   const std::string &iconPath,
                   const std::string &clickedIconPath)
    : UIElement(x, y, w, h), text(text), onClick(onClick), iconTexture(nullptr),
      clickedIconTexture(nullptr), currentTexture(nullptr) {

  if (!iconPath.empty()) {
    loadIconTexture(renderer, iconPath, &iconTexture);
    currentTexture = iconTexture;
    if (!iconTexture) {
      std::cerr << "Icon texture not loaded properly\n";
    }
  }
...
UIButton::~UIButton() {
  ...
  if (clickedIconTexture) {
    SDL_DestroyTexture(clickedIconTexture);
  }
}

bool UIButton::loadIconTexture(SDL_Renderer *renderer, const std::string &path,
                               SDL_Texture **texture) {
...
void UIButton::render(SDL_Renderer *renderer) {
  if (currentTexture != nullptr) {
    ...
    SDL_SetRenderDrawColor(renderer, isHovered ? 150 : 100,
                           isHovered ? 150 : 100, isHovered ? 255 : 255, 255);
    ...
    TTF_Font *font =
        TTF_OpenFont("/usr/share/fonts/TTF/JetBrainsMonoNerdFont-Bold.ttf", 24);
    if (font) {
      ...
      SDL_Rect textRect = {x + (width - textWidth) / 2,
                           y + (height - textHeight) / 2, textWidth,
                           textHeight};
