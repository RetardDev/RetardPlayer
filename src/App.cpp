...
App::App(int *argc, char ***argv)
    : argc(argc), argv(argv), window(nullptr), renderer(nullptr),
      isRunning(true) {
  ...
  if (SDL_Init(SDL_INIT_AUDIO) < 0) {
    std::cerr << "SDL could not initialise SDL ERROR: " << std::endl;
    return;
  }

  window = SDL_CreateWindow("Retard Player", SDL_WINDOWPOS_CENTERED,
