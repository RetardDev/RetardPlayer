...
bool VideoProcessor::openVideo(const char* filename){
  ...
  for(unsigned i = 0; i < pFormatContext->nb_streams; i++){
    if(pFormatContext->streams[i]->codecpar->codec_type ==AVMEDIA_TYPE_VIDEO){videoStreamIndex = i; break;}
  }
  if(videoStreamIndex == -1){std::cerr << "could not find a video stream\n"; return 0;}
  ...
  av_image_fill_arrays(pFrameYUV->data, pFrameYUV->linesize, buffer, AV_PIX_FMT_YUV420P, pCodecContext->width, pCodecContext->height, 32);

  img_convet_ctx = sws_getContext(pCodecContext->width, pCodecContext->height, pCodecContext->pix_fmt, pCodecContext->width, pCodecContext->height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);
  if(!img_convet_ctx){std::cerr << "Could not intialise sws contextn"; return 0;}
 ...
 pPacket = av_packet_alloc();
  ...
  AVRational fps = pCodecContext->framerate;
  if(fps.num == 0 || fps.den  == 0){std::cerr << "\nInvalid frame rate:\n"<< "fps.num: " << fps.num << ",den: " << fps.den << std::endl;
  return 0;}


  double frameRate = static_cast <double>(fps.num) / fps.den;
  if(frameRate == 0){std::cerr << "Frame rate can not be zero\n"; return 0;}
...
AVFrame* VideoProcessor::decodeFrame(){
  ...
  if(ret < 0){
    std::cerr << "Errror sending packet to decorder\n";
    return nullptr;
  }
  ...
  if(ret == AVERROR(EAGAIN) || ret == AVERROR_EOF){
    ...
    std::cerr << "Error receiving frame decoder\n";
    return nullptr;
  }

  if(pFrame->width == 0 || pFrame->height == 0){
    std::cerr << "Invalid source frame dimesions: " << pFrame->width << "x"<< pFrame->height << std::endl;
    return nullptr;
  }
  ...
  if(!img_convet_ctx){
    std::cerr << "Coudl not intialise the conversion context\n";
    return nullptr;
  }
}
